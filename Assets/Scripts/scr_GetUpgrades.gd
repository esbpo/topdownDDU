...
func Upgrade():
	...
	if upgradesAvailable >= 3: #As long as there are 3 or more upgrades not at max level, make all 3 upgrades different.
...
func levelIncrease (id):
	upgradeArray[id]["level"] += 1
	if upgradeArray[id]["level"] == upgradeArray[id]["maxlevel"]:
		upgradesAvailable -= 1
